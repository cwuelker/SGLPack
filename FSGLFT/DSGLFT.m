...
function F_Fourier = DSGLFT (B, F, path)
...
filename_b = sprintf ('%s/FSFT/precomp/%d/%d_100_digits_weights.txt', path, B, B);
...
a = half_range_Hermite_quadrature_adapted_weights_precomp (2 * B, path);

F_Fourier = zeros (B, B, 2 * B - 1, B);
   ...
   for k = 0 : 2 * B - 1    
    ...
    for m = - l : l
     ...
     for j = 0 : 2 * B - 1
      ...
      R_nl_i = R_mex (n, l, r(i + 1)) * exp (- r(i + 1) ^ 2); 
      
      F_Fourier(n, l + 1, m + B) = F_Fourier(n, l + 1, m + B) + a(i + 1) * b(j + 1) * F(j + 1, k + 1, i + 1) * R_nl_i * P_l_all_m_all_theta(m + l + 1, j + 1) * exp (- 1j * m * phi(k + 1));
...
for l = 0 : B - 1
 for m = - l : l
  ...
  for n = l + 1 : B
     ...
     F_Fourier(n, l + 1, m + B) = pi * F_Fourier(n, l + 1, m + B) * N(counter) / B;
